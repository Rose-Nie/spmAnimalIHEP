function obj = pr_basic_ui6(imgs, dispf)
...
% img_structure = fullfile(spm('Dir'),'toolbox', 'spmAnimalIHEP','replace_mouse','canonical','fsingle_brain.img');
img_structure = fullfile(spm('Dir'),'toolbox', 'spmAnimalIHEP','replace_mouse','canonical','TMBTA_Brain_Template_inPaxinos_spaceMIP.nii');
if ischar(imgs)
  imgs = cellstr(imgs);
end
if nargin < 2
...
if nimgs ==1
  ...
  [mx mn] = slover('volmaxmin', obj.img(2).vol);
    ...
    name_new = strcat(path,'\',name,'_axialT.ps');
    pr_print(name_new);
