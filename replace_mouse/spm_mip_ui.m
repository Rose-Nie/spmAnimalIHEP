...
% DXYZ = [182 218 182];
% CXYZ = [091 127 073];% human
% DXYZ = [166 234 198];
% CXYZ = [093 113 089]; % rat
DXYZ = [182 220 196];
CXYZ = [107 143 130]; %mouse
scale = 0.8;
...
%==========================================================================
    ...
    %======================================================================
        ...
        % 将读到的坐标值（即Paxinos坐标值）转换为矩阵坐标
        % 转置矩阵
        % nbb_rat_xyztemp1 = xyz;
        % nbb_rat_xyztemp = xyz + [0 0 6.96]';
        nbb_rat_xyztemp = xyz;
        MD  = get(h,'UserData');
        if nargin<4
            ...
            nbb_rat_xyz_kuochong(:,1:3) = nbb_rat_xyz_zhuanzhi;
            % 与Paxinos坐标转换到matrix坐标的系数相乘
            ptm = [-57.8109 0.1902 -0.0009; -0.0388  -56.0830 0.0000; -0.8011 -4.9715 4.1664; 464.0555  637.1833   66.4928];
            nbb_rat_xyz_Matrixtemp = nbb_rat_xyz_kuochong * ptm;
            % 除以系数 x/7.725; y/8.25
            nbb_rat_xyz_xishu = ones(sizenbbxyz(1),3);
            nbb_rat_xyz_xishu(:,1) = 0.1294;
            nbb_rat_xyz_xishu(:,2) = 0.1212;
            % 将扩充后的矩阵与系数矩阵点乘
            nbb_rat_xyz_enlarge = nbb_rat_xyz_Matrixtemp .* nbb_rat_xyz_xishu;
            sizenbbenlarge = size(nbb_rat_xyz_enlarge);
            nbb_rat_xyz_enlarge_new = nbb_rat_xyz_enlarge;
            for loopnbbx = 1:sizenbbenlarge(1)
                nbb_rat_xyz_enlarge(loopnbbx,2) = nbb_rat_xyz_enlarge_new(loopnbbx,3);
                nbb_rat_xyz_enlarge(loopnbbx,3) = nbb_rat_xyz_enlarge_new(loopnbbx,2);
            end
            % 转置Matrix坐标
            nbb_rat_xyz_Matrix = nbb_rat_xyz_enlarge';

            % M系数与大鼠脑地图集对应，目前还不知道如何自动获取M的值。
            M = [-1.0000 0 0 60.0000; 0 1.5000 0 -73.5000; 0 0 1.0000 -40.0000; 0 0 0 1.000];
            clear xyz;
            xyz = M(1:3,:)*[nbb_rat_xyz_Matrix; ones(1,size(nbb_rat_xyz_Matrix,2))];
        else
            xyz = MD.Md(1:3,:)*[xyz;1];
        end
...
%         spm_mip_ui('PosnMarkerPoints',MD.Md(1:3,:)*[xyz;1],h,'r');
        ...
        %------------------------------------------------------------------
        if ~isempty(MD.hReg) && MD.hReg~=hC, spm_XYZreg('SetCoords',nbb_rat_xyztemp,MD.hReg,h); end
    ...
    %======================================================================
        ...
        set(MS.hX(1),'Position',[ Po(1) + xyz2(2)/scale, Po(2) + xyz2(1)/scale, 0])
        set(MS.hX(2),'Position',[ Po(1) + xyz2(2)/scale, Po(3) - xyz2(3)/scale, 0])
        set(MS.hX(3),'Position',[ Po(4) + xyz2(1)/scale, Po(3) - xyz2(3)/scale, 0])
