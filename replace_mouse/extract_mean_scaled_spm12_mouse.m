function extract_mean_scaled_spm12_mouse
%EXTRACT_MEAN_SCALED Summary of this function goes here
%  Detailed explanation goes here
% file_roi = spm_get(Inf,'*.img','Select the ROI ...');
str_roi = 'D:\ROIs\mousebrain\ROIs_MIP_LR\';
% str_indi = 'F:\paper_data\pick_ROI\';
file_roi = dir(fullfile(str_roi,'*.nii'));
n = size(file_roi,1);
% file_indi = spm_get(Inf,'*.img','Select the individual images ...');
% m_indi = size(file_indi,1);
file_res = spm_select(Inf,'nii$','Select the result image ...');
% file_res = dir(fullfile(str_indi,'*.img'));
m = size(file_res,1);
tic
...
%             fprintf(id,'\r\n%s\t%s\t%s\t%s\t%s\t%s\r\n',outstr_qu,str_ke_qu,strmax_qu,strx_qu,stry_qu,strz_qu);
           ...
           for j = 1:n
            [pathr,namer,extr] = fileparts(file_roi(j,:).name);
