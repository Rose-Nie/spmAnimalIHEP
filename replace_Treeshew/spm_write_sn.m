...
% -----------------------------------------------
% 若为写出文件，则保存写出的文件名称
% V的形式为‘I:\data\PET\test\CON-B-32n.test.img,1’，需要只将图像路径及文件名的部分提取出来
% nbb ---- 2011.11.18
if nargout == 0
    if ~isempty(spmratIHEPsets)
        if spmratIHEPsets.flag == 1 && spmratIHEPsets.normaliseflag == 1
            spmratIHEPsets.writefilelength = size(V);
            writename_temp_nbb = zeros(spmratIHEPsets.writefilelength);
            writename_temp_nbb = char(writename_temp_nbb);
            for loop_nbb_ihep = 1:spmratIHEPsets.writefilelength(1)
                [path,name] = fileparts(V(loop_nbb_ihep,:));
                namestrtemp_nbb = strcat(path,'\',name,'.img');
                length_temp_nbb = length(namestrtemp_nbb);
                writename_temp_nbb(loop_nbb_ihep,1:length_temp_nbb) = namestrtemp_nbb;
            end
            spmratIHEPsets.writename = writename_temp_nbb;
            spmratIHEP_beforewrite(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1));
        end
    end
end
% -----------------------------------------------
...
%--------------------------------------------------------------------------
% 对标准化后的数据进行后处理，主要是使用各个脑模板对应的mask图像将标准化后的脑外像素去除
% 生成cw开头的去除脑外像素后的图像文件。
% nbb -----2011.11.18
if nargout == 0
    if ~isempty(spmratIHEPsets)
        if spmratIHEPsets.flag == 1 && spmratIHEPsets.normaliseflag == 1
            %---------------------------------------------
            % 判断选择的待处理的数据类型
            % spmratIHEPsets.datatype = 2 --- MRI_T2WI_surface
            %                           3 --- MRI_T2WI_birdcage
            %                           4 --- MRI_BOLD_surface
            %                           5 --- MRI_BOLD_birfcage
            %                           6 --- PET_FDG
            ...
            %                           0 --- error
            % nbb 2011.11.18
            switch spmratIHEPsets.datatype
                case 2
                ...
                case 6
                    IHEPratPET_afternorm(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1)); 
                    spmratIHEPsets.normaliseflag = 0;
                    IHEPratPET_alignatlas(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1));
                    IHEPratPET_adaptoMIP(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1));
                    spmratIHEPsets.normaliseflag = 1;
                case 7
                    IHEPratPET301_afternorm(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1)); 
                    spmratIHEPsets.normaliseflag = 0;
                    IHEPratPET301_alignatlas(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1));
                    IHEPratPET_adaptoMIP(spmratIHEPsets.writename,spmratIHEPsets.writefilelength(1));
                    spmratIHEPsets.normaliseflag = 1;                   
                case 0
                    error('Please choose a datatype to be normalised.');
            end
        end
    end
end
