function IHEP_old_normalization(Template,SourceImage,ImagetoWrite)
% List of open inputs
...
Fullpath = fullfile(spm('Dir'),'toolbox', 'spmAnimalIHEP','replace_ZebraFinch','IHEP_old_normalization_job.m');
jobfile = {Fullpath};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(3, nrun);
for crun = 1:nrun
    inputs{1, crun} = cellstr(SourceImage(crun,:)); % Old Normalise: Estimate & Write: Source Image - cfg_files
    inputs{2, crun} = cellstr(ImagetoWrite(crun,:)); % Old Normalise: Estimate & Write: Images to Write - cfg_files
