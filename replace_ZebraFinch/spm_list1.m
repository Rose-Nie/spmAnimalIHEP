...
% FORMAT TabDat = spm_list('list',SPM,hReg)
    ...
    matrixnbb = [159.2  117.6000  120];
    voxelsizenbb = [abs(M(1,1)) abs(M(2,2)) abs(M(3,3))];
    Newnbb_x = 1:voxelsizenbb(1):matrixnbb(1);
    Newnbb_y = 1:voxelsizenbb(2):matrixnbb(2);
    Newnbb_z = 1:voxelsizenbb(3):matrixnbb(3);
    XYZTEMP = [round(Newnbb_x(XYZ(1,:))-0.5); round(Newnbb_y(XYZ(2,:))-0.5); round(Newnbb_z(XYZ(3,:))-0.5)];
    XYZ = XYZTEMP;
    ...
    % 转置矩阵
    nbb_rat_xyztemp = XYZ;
    nbb_rat_xyz_zhuanzhi = nbb_rat_xyztemp';
    % 将矩阵加一列1，为了保证常数系数的相乘
    sizenbbxyz = size(nbb_rat_xyz_zhuanzhi);
    nbb_rat_xyz_zhuanzhi_new = nbb_rat_xyz_zhuanzhi;
    for loopnbbx = 1:sizenbbxyz(1)
        nbb_rat_xyz_zhuanzhi_new(loopnbbx,2) = nbb_rat_xyz_zhuanzhi(loopnbbx,3);
        nbb_rat_xyz_zhuanzhi_new(loopnbbx,3) = nbb_rat_xyz_zhuanzhi(loopnbbx,2);
    end
    nbb_rat_xyz_kuochong = ones(sizenbbxyz(1),4);
    nbb_rat_xyz_kuochong(:,1:3) = nbb_rat_xyz_zhuanzhi_new;
    % 设置x y的常数系数，x*7.725;y*8.25 rat
    % 小鼠的直接用的缩小的矩阵与图谱做的对应，所以不用扩大倍数。
    nbb_rat_xyz_xishu = ones(sizenbbxyz(1),4);
    nbb_rat_xyz_xishu(:,1) = 1;
    nbb_rat_xyz_xishu(:,2) = 1;
    % 将扩充后的矩阵与系数矩阵点乘
    nbb_rat_xyz_enlarge = nbb_rat_xyz_kuochong .* nbb_rat_xyz_xishu;
    % 与matrix坐标转换到Paxinos坐标的系数相乘
%       A = [0.0801 -0.001 -0.0009;-0.0015 -0.0858 -0.0009;0.0002 -0.0130 0.0724;-7.6588 12.3166 -2.9928];
    mtp = [0.0801 -0.001 -0.0009;-0.0015 -0.0858 -0.0009;0.0002 -0.0130 0.0724;-7.6588 12.3166 -2.9928];
    nbb_rat_xyz_Paxinos =  nbb_rat_xyz_enlarge * mtp;
    % nbb_rat_xyz_Paxinos_dim = size(nbb_rat_xyz_Paxinos);
    % for nbb_rat_xyz_Paxinos_x = 1:nbb_rat_xyz_Paxinos_dim(1)
    %     nbb_rat_xyz_Paxinos_temp(nbb_rat_xyz_Paxinos_x,:) =
    %     nbb_rat_xyz_Paxinos(nbb_rat_xyz_Paxinos_x,:) + [0 0 -6.96];
    % end
    % nbb_rat_xyz_Paxinos = nbb_rat_xyz_Paxinos_temp;
    % 转置Paxinos坐标
    XYZrat = nbb_rat_xyz_Paxinos';
    % XYZrat = XYZmm;
...
%         h      = text(tCol(12),y,sprintf(TabDat.fmt{12},tXYZmm),...
        h     = text(tCol(12),y,sprintf(TabDat.fmt{12},XYZrat(:,i)),...
            'FontWeight','Bold',...
            'Tag','ListXYZ',...
            'ButtonDownFcn',[...
