function obj = pr_basic_ui1(imgs, dispf)
...
img_structure = fullfile(spm('Dir'),'toolbox', 'spmAnimalIHEP','replace_ZebraFinch','canonical','BrainZebraFinchT2TemplateMIP_n.nii');
if ischar(imgs)
  imgs = cellstr(imgs);
end
if nargin < 2
  ...
  [mx mn] = slover('volmaxmin', obj.img(2).vol);
    ...
    if strcmp(itype,'Structural with SPM blobs')
      obj = add_spm(obj);
        ...
        slices = obj.slices;
        obj.slices = -39:1.6:46;
        % and do the display
        if dispf, obj = paint(obj); end
        [path,name,ext] = fileparts(imgs{1});
        name_new = strcat(path,'\',name,'_coronal.ps');
        pr_print(name_new);
