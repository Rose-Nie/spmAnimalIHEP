...
%     dScale = 1;
	...
	xdim = max(1,floor((abs(dim(1) / dScalecx) + 0.99)/2));
	ydim = max(1,floor((abs(dim(2) / dScalecy) + 0.99)/2));
	zdim = max(1,floor((abs(dim(3) / dScalecz) + 0.99)/2)); 
	m    = floor((dim(4)));
	m1   = floor((dim(5)));
	...
	if (m == DY+DX && m1 == DZ+DX) 
		...
		for i=1:(n-1);
            ...
            x = floor(l(1,i)/dScalecx)+CX;
            y = floor(l(2,i)/dScalecy)+CY;
            z = floor(l(3,i)/dScalecz)+CZ;

			if (2*CX-x-xdim>0 && 2*CX-x+xdim<DX && y-ydim>0 && y+ydim<DY) %ºá¶ÏÃæ
				q = v(i);
			...
			if (z-zdim>0 && z+zdim<DZ && y-ydim>0 && y+ydim<DY) %Ê¸×´Ãæ
				q = v(i);
				for j = -ydim:ydim
					for k = -zdim:zdim
						%o = 1+j + y + (DX + k + z)*m;
			...
			if (x-xdim>0 && x+xdim<DX && z-zdim>0 && z+zdim<DZ) %¹Ú×´Ãæ
				q = v(i);
				for j = -xdim:xdim
					for k = -zdim:zdim
						%o = 1+DY + j + x + (DX + k + z)*m;
