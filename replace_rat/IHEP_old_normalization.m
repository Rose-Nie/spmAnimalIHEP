...
Fullpath = fullfile(spm('Dir'),'toolbox', 'spmAnimalIHEP','replace_rat','IHEP_old_normalization_job.m');
jobfile = {Fullpath};
...
%     inputs{2, crun} = cellstr(ImagetoWrite); % Old Normalise: Estimate & Write: Images to Write - cfg_files
    inputs{2, crun} = cellstr(ImagetoWrite(crun,:)); % Old Normalise: Estimate & Write: Images to Write - cfg_filess
