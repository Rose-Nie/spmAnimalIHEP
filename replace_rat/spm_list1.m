...
% FORMAT TabDat = spm_list('list',SPM,hReg)
    ...
    %----------------------------------------------------------------------
%     matrixnbb = [120 147 80];
    matrixnbb = [140 147 90]; %130的数据
    ...
    % 设置x y的常数系数，x*7.725;y*8.25
    nbb_rat_xyz_xishu = ones(sizenbbxyz(1),4);
    nbb_rat_xyz_xishu(:,1) = 7.725;
    nbb_rat_xyz_xishu(:,2) = 8.25;
    % 将扩充后的矩阵与系数矩阵点乘
    
    nbb_rat_xyz_kuochong(:,1) = nbb_rat_xyz_kuochong(:,1) -10; %130后的数据

    nbb_rat_xyz_enlarge = nbb_rat_xyz_kuochong .* nbb_rat_xyz_xishu;
    % 与matrix坐标转换到Paxinos坐标的系数相乘
    mtp = [-0.0173 -0.001 0; 0 -0.0177 0; -0.0033 -0.0213 0.24; 8.2266 12.7566 -15.9579];
