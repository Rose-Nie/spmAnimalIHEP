...
m = size(file_res,1);
tic
A = [-0.0173 -0.001 0; 0 -0.0177 0; -0.0033 -0.0213 0.24; 8.2266 12.7566 -15.9579];
Matrix_mouse = [140 147 90];
pathre_backup = '';
...
%     resultname = strcat(str_indi,file_res(k,:).name);
    ...
    ynbb = int16(round(Matrix_resultimg_y - 0.5)/1.5);
    znbb = int16(round(Matrix_resultimg_z - 0.5));    
        ...
        for i = 1:n_qu
            ...
            matrix_qu = [(cor(1)-10)*7.725  cor(3)*8.25  cor(2)  1];
...
%             roiimg(temp1) = 1;
            ...
            if ~isempty(temp)
                ...
                matrix = [(cor1(1)-10)*7.725  cor1(3)*8.25  cor1(2)  1];
